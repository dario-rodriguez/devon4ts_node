...
import { IDevon4nodeApplicationOptions } from '../../../src/lib/devon4node-application/devon4node-application.factory';

describe('Auth Factory', () => {
  const runner = new SchematicTestRunner('.', path.join(__dirname, '../../../src/collection.json'));
  ...
  it('should throw an error if not executed at project root folder', async () => {
    try {
      await runner.runSchematic('auth-jwt', {});
      fail();
    } catch (error) {
  ...
  it('should generate all files required for auth with JWT', async () => {
    let tree = await runner.runSchematic('application', appOptions);
    tree = await runner.runSchematic('auth-jwt', {}, tree);
  ...
  it('should import AuthModule and UserModule in CoreModule', async () => {
    ...
    const files: string[] | undefined = tree?.files;
