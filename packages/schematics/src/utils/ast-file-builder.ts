    ...
    if (varDeclaration) {
      ...
      if (object) {
        ...
        if (arg && arg.getKind() === SyntaxKind.ObjectLiteralExpression) {
          ...
          if (property) {
            if (property.getKind() === SyntaxKind.ArrayLiteralExpression) {
              if (Array.isArray(propertyInitializer)) {
                propertyInitializer.forEach(elem => {
                  (property as unknown as ArrayLiteralExpression).addElement(elem);
                });
              } else {
                (property as unknown as ArrayLiteralExpression).addElement(propertyInitializer);
              }
            } else {
              property.set({
                initializer: Array.isArray(propertyInitializer)
                  ? JSON.stringify(propertyInitializer)
                  : propertyInitializer,
              });
            }
          } else {
